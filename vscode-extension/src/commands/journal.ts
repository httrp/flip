...
export async function createJournal(): Promise<void> {
  ...
  // Ask user for date with a small date picker (default: today)
  const today = new Date();
  const dateStr = today.toISOString().split('T')[0]; // YYYY-MM-DD
  const selectedDate = await pickDate(dateStr);
  if (!selectedDate) {
    return; // User cancelled
  }
...
/**
 * Show a lightweight webview-based date picker
 */
async function pickDate(defaultDate: string): Promise<string | undefined> {
