...
func getVSCodeTasksJSON() string {
	json, err := assets.GetVSCodeTasksJSON(TasksVersion)
	if err != nil {
		// Fallback should never happen with embedded files
		return "{}"
	}
	return json
}
...
func NewVSCodeCommand() *cobra.Command {
	...
	cmd.AddCommand(NewVSCodeTemplatesCommand())
...
func installTasksGlobal() error {
	...
	// Fresh install
	if err := os.WriteFile(tasksPath, []byte(getVSCodeTasksJSON()), 0644); err != nil {
...
func mergeTasksFile(tasksPath string, isGlobal bool) error {
	...
	if err := json.Unmarshal([]byte(getVSCodeTasksJSON()), &flipTasks); err != nil {
