...
func selectBrainForOperation(promptLabel string) (*Brain, error) {
    ...
    items := make([]string, 0, len(ws.Brains))
    indexToBrain := make(map[int]*Brain)
    for i := range ws.Brains {
        ...
        items = append(items, fmt.Sprintf("%s%s  —  %s", marker, b.Name, b.Path))
        indexToBrain[i] = &ws.Brains[i]
    }
